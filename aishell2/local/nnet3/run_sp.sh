...
if [ $stage -le 1 ]; then
  ...
  local/data/perturb_data_dir_speed_3way.sh --mode tempo data/${train_set} data/${train_set}_sp
...
if [ $stage -le 3 ]; then
  echo "$0: creating fbank features for NN training"
  fbankdir=fbank_hires
  for datadir in ${train_set}_sp ${test_sets}; do
  	utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    utils/data/perturb_data_dir_volume.sh data/${datadir}_hires || exit 1;
  	steps/make_fbank.sh --fbank-config conf/fbank.16k.conf --nj $nj data/${datadir}_hires exp/make_fbank/ $fbankdir
  done
fi
exit
if [ $stage -le 3 ]; then
